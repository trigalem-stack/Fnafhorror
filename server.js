    ...
    socket.on('hitEntity', (data) => {
        ...
        if(data.type === 'tree') {
            ...
            if (tree) {
                ...
                if (tree.hp <= 0) {
                    tree.isCut = true;
                    // Раскидываем бревна с небольшим разбросом, чтобы они не падали друг в друга (имитация коллизии)
                    for (let k = 0; k < 3; k++) {
        ...
        if (data.type === 'log') {
            ...
            if (index !== -1) {
                if (data.toBag && p.bag.length < 10) {
                    ...
                    lobby.world.droppedSaplings[index].collected = true; 
                    socket.emit('itemToSlot', 'sapling');
                }
            }
        }
    });
    ...
    function handlePlayerLeave(s) {
        if (currentLobby && lobbies[currentLobby]) {
            ...
            if (lobby.players.length === 0) {
                // Удаление локации/лобби если вышли ВСЕ игроки
...
function startGameTicker(lobbyId) {
    ...
    const interval = setInterval(() => {
        ...
        lobby.world.wolves.forEach((w, index) => {
            // КОЛЛИЗИЯ ВОЛКОВ друг с другом (чтобы не слипались)
            ...
            if(closestP) {
                ...
                if (cf.hp > 0 && dToFireW < cfRadius) {
                    ...
                    if(minD > 1.2) {
                        ...
                        // Перезарядка укуса ВОЛКА изменена на 3 секунды (3000 мс)
