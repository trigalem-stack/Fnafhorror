...
io.on('connection', (socket) => {
    ...
    socket.on('createLobby', (data) => {
        ...
        lobbies[lobbyId] = {
            ...
            world: {
                ...
                isNight: false, 
                timeProgress: 0,
                logCounter: 0, // Оптимизация счетчика ID
                campfire: {
                    x: 0,
                    z: 0,
                    hp: 100,
    ...
    // Новая логика броска без физики импульса (сразу вниз перед собой)
    socket.on('dropLog', () => {
        if (!currentLobby || !lobbies[currentLobby]) return;
        const lobby = lobbies[currentLobby];
        const p = lobby.players.find(pl => pl.id === socket.id);
        ...
        let dist = 0.8; // Расстояние перед игроком
        ...
        lobby.world.droppedLogs.push({
            id: 'log_' + lobby.world.logCounter,
            x: lx,
            y: 1.0, // Падает сверху (почти с уровня рук)
            z: lz,
            vx: 0, // Без физики полета
            vz: 0, 
            collected: false
        });
    });
    ...
    function handlePlayerLeave(socket) {
        if (currentLobby && lobbies[currentLobby]) {
            ...
            if (lobby.players.length === 0) {
                delete lobbies[currentLobby]; // Очистка памяти сервера если никого нет
            } else {
                io.to(lobby.id).emit('lobbyUpdated', lobby);
...
function startGameTicker(lobbyId) {
    ...
    const interval = setInterval(() => {
        ...
        // Костер сгорает только если HP > 0
        if (cf.hp > 0) {
            let tickRate = 1000; 
            ...
            lastCampfireTick = now; // Чтобы при розжиге он не сжег часть HP мгновенно
        }
        ...
        lobby.world.droppedLogs.forEach(log => {
            ...
            // ИСПРАВЛЕНА ЛОГИКА КОСТРА: Теперь разгорается даже с 0 HP
            if (!log.collected) {
                let dx = log.x - cf.x;
                let dz = log.z - cf.z;
                if (Math.sqrt(dx*dx + dz*dz) < 1.5 && log.y < 1.0) {
                    ...
                    if (cf.hp >= 100 && cf.level < 6) {
                        cf.level += 1;
                        cf.hp = cf.hp - 100;
                        if(cf.hp === 0) cf.hp = 1; // Предотвращаем баг, когда сразу тухнет
                    } else if (cf.hp > 100) {
                        cf.hp = 100; 
                    }
                }
