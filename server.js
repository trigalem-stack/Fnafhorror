...
io.on('connection', (socket) => {
    ...
    // Изменена физика броска дров: падает перед игроком, а не летит далеко
    socket.on('dropLog', () => {
        if (!currentLobby || !lobbies[currentLobby]) return;
        const lobby = lobbies[currentLobby];
        const p = lobby.players.find(pl => pl.id === socket.id);
        ...
        let dist = 0.8; // Бревно появляется прямо перед игроком
        ...
        lobby.world.droppedLogs.push({
            id: 'log_' + Date.now() + '_' + Math.random(),
            x: lx,
            y: 0.8, // Падает сверху, но не слишком высоко 
            z: lz,
            vx: -Math.sin(p.ry) * 0.05, // Почти нулевой импульс
            vz: -Math.cos(p.ry) * 0.05,
            collected: false
        });
    });
                ...
                if (Math.sqrt(dx*dx + dz*dz) < 1.5 && log.y < 1.0) {
                    ...
                    if (cf.hp > 0) {
                        ...
                        if (cf.hp >= 100 && cf.level < 6) {
                            cf.level += 1;
                            cf.hp -= 100;
                            // ФИКС БАГА С 2 ЛВЛОМ: Если HP ровно упало до 0 при переходе, оно "сдохло". Защищаем костер!
                            if (cf.hp <= 0) cf.hp = 1; 
                        } else if (cf.hp > 100) {
                            cf.hp = 100; 
                        }
