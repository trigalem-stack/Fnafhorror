...
io.on('connection', (socket) => {
    ...
    socket.on('createLobby', (data) => {
        ...
        lobbies[lobbyId] = {
            ...
            isPlaying: false,
            world: {
                day: 1, // Начинаем с 1 дня
                isNight: false, // Начинаем с утра
                timeProgress: 0,
                campfire: {
                    x: 0,
                    z: 0,
    ...
    socket.on('dropLog', (actionTarget) => {
        ...
        if (actionTarget === 'campfire') {
            ...
            if (cf.hp > 0) {
                // Новая логика ХП костра (Чем выше уровень, тем меньше ХП дает бревно)
                let hpGain = 20; // 1 лвл
                ...
                if (cf.hp >= 85 && cf.level < 6) {
                    cf.level += 1;
                    cf.hp = 100;
                } else {
                    cf.hp = Math.min(100, cf.hp + hpGain);
                }
            }
        } else {
            let dist = 2;
...
function startGameTicker(lobbyId) {
    ...
    const interval = setInterval(() => {
        ...
        let cf = lobby.world.campfire;
        if (cf.hp > 0) {
            // Новая скорость костра: 1 лвл = 1000мс (1 секунда). Выше уровень - больше времени
            let tickRate = 1000; 
