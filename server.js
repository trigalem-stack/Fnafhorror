...
io.on('connection', (socket) => {
    ...
    socket.on('hitTree', (treeId) => {
        ...
        if (tree) {
            ...
            if (tree.hp <= 0) {
                tree.isCut = true;
                for (let k = 0; k < 3; k++) {
                    // Проверка лимита: удаляем старые бревна, если перебор
    ...
    socket.on('collectLog', (logId) => {
        ...
        if (logIndex !== -1) {
            lobby.world.droppedLogs[logIndex].collected = true; // Помечаем, уберется в тикере
            p.bagCount += 1;
            socket.emit('bagSync', p.bagCount);
        }
    });

    // Изменена логика броска: Бревно всегда летит вперед с физикой
    socket.on('dropLog', () => {
        if (!currentLobby || !lobbies[currentLobby]) return;
        const lobby = lobbies[currentLobby];
        const p = lobby.players.find(pl => pl.id === socket.id);
        ...
        lobby.world.droppedLogs.push({
            ...
            y: 1.5, // Падает сверху 
            z: lz,
            vx: -Math.sin(p.ry) * 0.4, // Придаем импульс
            vz: -Math.cos(p.ry) * 0.4,
            collected: false
        });
    });

    socket.on('kickPlayer', (playerId) => { /* ... (без изменений) ... */ });
    socket.on('disbandLobby', () => { /* ... (без изменений) ... */ });
    socket.on('leaveLobby', () => { /* ... (без изменений) ... */ });
...
function startGameTicker(lobbyId) {
    ...
    const interval = setInterval(() => {
        ...
        // Физика бревен и коллизия с костром
        lobby.world.droppedLogs.forEach(log => {
            if (log.y > 0.2) {
                log.y -= 0.1; // Гравитация
                log.x += log.vx;
                log.z += log.vz;
                log.vx *= 0.95; // Трение воздуха
                log.vz *= 0.95;
            } else {
                log.y = 0.2;
                log.vx *= 0.8; // Трение земли
                log.vz *= 0.8;
            }

            // Коллизия с костром (если бревно близко и еще не собрано)
            if (!log.collected) {
                let dx = log.x - cf.x;
                let dz = log.z - cf.z;
                if (Math.sqrt(dx*dx + dz*dz) < 1.5 && log.y < 1.0) {
                    log.collected = true; // Сгорает
        ...
        // Убираем сгоревшие или собранные бревна
        ...
        io.to(lobbyId).emit('worldSync', {
            ...
            })) // Округление для экономии пакетов
        });
