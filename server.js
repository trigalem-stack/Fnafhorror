...
const MAP_SIZE = 100;
const MAX_SPEED = 6.0; // Максимальная скорость для античита (чуть больше базовой 4.5)
...
io.on('connection', (socket) => {
    ...
    socket.on('createLobby', (data) => {
        // Проверка на уникальность имени лобби
        ...
        lobbies[lobbyId] = {
            ...
            world: {
                ...
                trees: trees, droppedLogs: [], saplings: [], wolves: [], droppedSaplings: []
            }
        };
    ...
    socket.on('playerUpdate', (data) => {
        ...
        if (p && p.hp > 0) {
            ...
            if (dt > 0.05) {
                ...
                const tolerance = (data.ping && data.ping > 150) ? 2.0 : 0.5; // Большой пинг дает крошечную поблажку на рассинхрон
                
                if (dist / dt > MAX_SPEED + tolerance) {
                    // Телепорт назад (отклоняем перемещение)
                    socket.emit('forceMove', { x: p.x, z: p.z });
                    p.lastUpdate = now;
                    return; 
                }
            }
            p.lastUpdate = now; p.ping = data.ping || 50;
        ...
        if(data.type === 'tree') {
            ...
            if (tree) {
                ...
                if (tree.hp <= 0) {
                    ...
                    // Спавн саженца как 3D модель на пол
                    lobby.world.uidCounter++;
    ...
    // Сбор предметов с пола (Бревна и Саженцы)
    socket.on('collectItem', (data) => {
        ...
        if (data.type === 'log') {
            ...
            const index = lobby.world.droppedSaplings.findIndex(s => s.id === data.id && !s.collected);
            if (index !== -1) {
                if (data.toBag && p.bag.length < 10) {
                    lobby.world.droppedSaplings[index].collected = true; 
                    p.bag.push('sapling'); socket.emit('inventorySync', {bag: p.bag});
                } else if (!data.toBag) {
                    lobby.world.droppedSaplings[index].collected = true; 
                    socket.emit('itemToSlot', 'sapling'); // Клиент сам положит в пустой слот
                }
            }
        }
    });
    ...
    socket.on('dropItem', (data) => {
        ...
        if (data.fromBag && p.bag.length > 0) {
            let item = p.bag.pop(); // Достаем последний предмет
            socket.emit('inventorySync', {bag: p.bag});
            lobby.world.uidCounter++;
            if(item === 'log') {
                lobby.world.droppedLogs.push({ id: 'log_' + lobby.world.uidCounter, x: p.x - Math.sin(p.ry)*dist, y: 1.5, z: p.z - Math.cos(p.ry)*dist, collected: false });
            } else {
                lobby.world.droppedSaplings.push({ id: 'sap_' + lobby.world.uidCounter, x: p.x - Math.sin(p.ry)*dist, y: 1.5, z: p.z - Math.cos(p.ry)*dist, collected: false });
            }
        } else if (!data.fromBag && data.type === 'sapling') {
            lobby.world.uidCounter++;
            lobby.world.droppedSaplings.push({ id: 'sap_' + lobby.world.uidCounter, x: p.x - Math.sin(p.ry)*dist, y: 1.5, z: p.z - Math.cos(p.ry)*dist, collected: false });
        }
    });
                ...
                if (cf.hp > 0 && dToFireW < cfRadius) {
                    ...
                    if(minD > 1.2) {
                        ...
                        if (now - w.lastAttack > 2000) {
                            ...
                            if(closestP.hp <= 0) {
                                closestP.hp = 0; 
                                io.to(lobbyId).emit('chatMessage', `🐺 Волки растерзали ${closestP.name}`);
                            }
