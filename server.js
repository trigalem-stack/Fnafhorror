...
// ОГРАНИЧЕНИЕ НА МАКСИМАЛЬНОЕ КОЛИЧЕСТВО БРЕВЕН (Вопрос про лимит в >30 штук)
const MAX_DROPPED_LOGS = 30; // Ограничение для стабильной работы на Render.com
...
io.on('connection', (socket) => {
    ...
    socket.on('hitTree', (treeId) => {
        ...
        if (tree) {
            ...
            if (tree.hp <= 0) {
                tree.isCut = true;
                for (let k = 0; k < 3; k++) {
                    // ЕСЛИ БРЕВЕН УЖЕ БОЛЬШЕ 30, САМОЕ СТАРОЕ УДАЛЯЕТСЯ
    ...
    socket.on('dropLog', () => {
        ...
        // ЕСЛИ ИГРОК ВЫБРАСЫВАЕТ БРЕВНО И ЛИМИТ ПРЕВЫШЕН, УДАЛЯЕТСЯ СТАРОЕ
