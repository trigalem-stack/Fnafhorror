...
io.on('connection', (socket) => {
    ...
    socket.on('createLobby', (data) => {
        ...
        const trees = [];
        for (let i = 0; i < 60; i++) {
            let tx = (Math.random() - 0.5) * 30; let tz = (Math.random() - 0.5) * 30;
        ...
        lobbies[lobbyId] = {
            ...
            tps: 20, lastTickTime: Date.now(),
            world: {
                day: 1, isNight: false, timeProgress: 0, uidCounter: 0, lastRegenTime: Date.now(),
                mapLevel: 1, housesGenerated: false,
                campfire: { x: 0, z: 0, hp: 100, level: 1 },
                workbench: { x: 3, z: 3, wood: 0, iron: 0, bedCrafted: false, clockCrafted: false },
                recycler: { x: 1.8, z: 3 }, 
                trees: trees, droppedLogs: [], saplings: [], wolves: [], 
                droppedSaplings: [], beds: [], droppedBeds: [], 
                houses: [], droppedChairs: [], droppedScraps: [], clocks: []
            }
        };
    ...
    socket.on('craftItem', (data) => {
        if (!currentLobby || !lobbies[currentLobby]) return;
        const lobby = lobbies[currentLobby];
        if(data.type === 'bed' && lobby.world.workbench.wood >= 15 && !lobby.world.workbench.bedCrafted) {
            lobby.world.workbench.wood -= 15; lobby.world.workbench.bedCrafted = true;
            socket.emit('itemToSlot', 'bed'); 
        } else if (data.type === 'clock' && lobby.world.workbench.wood >= 4 && lobby.world.workbench.iron >= 2 && !lobby.world.workbench.clockCrafted) {
            lobby.world.workbench.wood -= 4; lobby.world.workbench.iron -= 2; lobby.world.workbench.clockCrafted = true;
            socket.emit('itemToSlot', 'clock'); 
        }
    });
    ...
    socket.on('collectItem', (data) => {
        ...
        } else if (data.type === 'sapling' || data.type === 'bed_item' || data.type === 'chair' || data.type === 'scrap') {
            let arr, bagType;
            if(data.type === 'sapling') { arr = lobby.world.droppedSaplings; bagType = 'sapling'; }
            else if(data.type === 'bed_item') { arr = lobby.world.droppedBeds; bagType = 'bed'; }
            else if(data.type === 'chair') { arr = lobby.world.droppedChairs; bagType = 'chair'; }
            else if(data.type === 'scrap') { arr = lobby.world.droppedScraps; bagType = 'scrap'; }

            const index = arr.findIndex(s => s.id === data.id && !s.collected);
            if (index !== -1) {
                if (data.toBag && p.bag.length < 10) {
                    arr[index].collected = true; p.bag.push(bagType); socket.emit('inventorySync', {bag: p.bag});
                } else if (!data.toBag) {
                    arr[index].collected = true; socket.emit('itemToSlot', bagType);
                }
            }
        } else if (data.type === 'placedbed' || data.type === 'placedclock') {
            let arr = data.type === 'placedbed' ? lobby.world.beds : lobby.world.clocks;
            let bagType = data.type === 'placedbed' ? 'bed' : 'clock';
            const index = arr.findIndex(b => b.id === data.id);
            if(index !== -1) {
                arr.splice(index, 1);
                if (data.toBag && p.bag.length < 10) {
                    p.bag.push(bagType); socket.emit('inventorySync', {bag: p.bag});
                } else {
                    socket.emit('itemToSlot', bagType);
                }
            }
        }
    });
    ...
    socket.on('dropItem', (data) => {
        ...
        if (data.fromBag && p.bag.length > 0) {
            ...
            else if (item === 'chair') lobby.world.droppedChairs.push(dObj);
            else if (item === 'scrap') lobby.world.droppedScraps.push(dObj);
            ...
            else if(data.type === 'chair') lobby.world.droppedChairs.push(dObj);
            else if(data.type === 'scrap') lobby.world.droppedScraps.push(dObj);
            else if(data.type === 'clock') lobby.world.clocks.push(dObj);
        }
    });
...
function generateHouses(lobby) {
    for(let i=0; i<5; i++) {
        lobby.world.uidCounter++;
        let hx, hz;
        // Спавним дома за пределами изначальной зоны (от 15 до 30 по радиусу)
        let angle = Math.random() * Math.PI * 2;
        let dist = 15 + Math.random() * 15;
        hx = Math.cos(angle) * dist; hz = Math.sin(angle) * dist;
        
        let house = { id: 'house_'+lobby.world.uidCounter, x: hx, z: hz, width: 6, depth: 6 };
        lobby.world.houses.push(house);

        // Спавн стульев внутри дома
        for(let c=0; c<3; c++) {
            lobby.world.uidCounter++;
            let cx = hx + (Math.random() - 0.5) * 4; let cz = hz + (Math.random() - 0.5) * 4;
            lobby.world.droppedChairs.push({ id: 'chair_'+lobby.world.uidCounter, x: cx, y: 0.5, z: cz, collected: false });
        }
        // Шанс 30% на железный лом
        if (Math.random() <= 0.30) {
            lobby.world.uidCounter++;
            lobby.world.droppedScraps.push({ id: 'scrap_'+lobby.world.uidCounter, x: hx, y: 0.5, z: hz, collected: false });
        }
    }
}
...
function startGameTicker(lobbyId) {
    const interval = setInterval(() => {
        ...
        const now = Date.now(); const dt = now - lobby.lastTickTime; lobby.lastTickTime = now;
        
        // Расчет TPS сервера
        lobby.tps = Math.round(1000 / dt);
        ...
        if (lobby.world.timeProgress >= DAY_NIGHT_DURATION) { 
            ...
            if(lobby.world.beds.length > 0) lobby.world.day += 2; else lobby.world.day += 1; 
        }
        ...
        if (now - lobby.world.lastRegenTime >= 5000) {
            lobby.players.forEach(p => {
                if (p.hp > 0 && p.hp < 100) {
                    ...
                    io.to(p.id).emit('playerDamaged', p.hp); 
                }
            });
        ...
        let activeItems = [...lobby.world.droppedLogs, ...lobby.world.droppedChairs, ...lobby.world.droppedScraps];
        ...
        activeItems.forEach(item => {
            ...
            if (!item.collected) {
                ...
                if (Math.sqrt(dxC*dxC + dzC*dzC) < 1.8 && item.y < 0.8) {
                    if(item.id.startsWith('log_') || item.id.startsWith('chair_')) {
                        item.collected = true; 
                        let gain = item.id.startsWith('chair_') ? 15 : Math.max(2, 20 - (cf.level * 3)); 
                        cf.hp += gain;
                        if (cf.hp >= 100 && cf.level < 10) { 
                            cf.level++; cf.hp -= 100; if(cf.hp===0) cf.hp=1; 
                            // Генерация карты и домов при 2 лвле костра
                            if(cf.level === 2 && !lobby.world.housesGenerated) {
                                lobby.world.mapLevel = 2;
                                lobby.world.housesGenerated = true;
                                generateHouses(lobby);
                            }
                ...
                if (!item.collected && Math.sqrt(dxR*dxR + dzR*dzR) < 1.5 && item.y < 0.8) {
                    item.collected = true;
                    if(item.id.startsWith('log_')) lobby.world.workbench.wood += 1;
                    if(item.id.startsWith('chair_')) lobby.world.workbench.wood += 3;
                    if(item.id.startsWith('scrap_')) lobby.world.workbench.iron += 1;
                }
            }
        });
        
        lobby.world.droppedLogs = lobby.world.droppedLogs.filter(l => !l.collected);
        lobby.world.droppedChairs = lobby.world.droppedChairs.filter(c => !c.collected);
        lobby.world.droppedScraps = lobby.world.droppedScraps.filter(s => !s.collected);
        ...
        io.to(lobbyId).emit('worldSync', {
            tps: lobby.tps, mapLevel: lobby.world.mapLevel,
            ...
            beds: lobby.world.beds, houses: lobby.world.houses, clocks: lobby.world.clocks,
            ...
            droppedScraps: lobby.world.droppedScraps.map(sc => ({ id: sc.id, x: Number(sc.x.toFixed(2)), y: Number(sc.y.toFixed(2)), z: Number(sc.z.toFixed(2)) })),
    ...
    }, 50); // Ускорил цикл до 50 мс = 20 TPS.
}
