...
io.on('connection', (socket) => {
    ...
    socket.on('hitTree', (treeId) => {
        ...
        if (tree) {
            tree.hp -= 1;
            // ИСПРАВЛЕНИЕ: Отправляем ивент всем для анимации удара
    ...
    socket.on('dropLog', (actionTarget) => {
        ...
        if (actionTarget === 'campfire') {
            ...
            if (cf.hp > 0) {
                // ИСПРАВЛЕНИЕ: Баг костра. Теперь HP прибавляется корректно и не летит до 6 уровня сразу.
                let hpGain = 20; 
                ...
                // Проверяем уровень только если перешагнули 100 ХП
                if (cf.hp >= 100 && cf.level < 6) {
                    cf.level += 1;
                    cf.hp = cf.hp - 100; // Оставляем "лишнее" хп на следующий уровень
                } else if (cf.hp > 100 && cf.level >= 6) {
                    cf.hp = 100; // Максимум ХП на максимальном уровне
                }
            }
        } else {
            let dist = 2;
    ...
    const interval = setInterval(() => {
        ...
        lobby.world.trees.forEach(t => {
            if (t.isCut && t.sinkY > -6) {
                // ИСПРАВЛЕНИЕ: Ускорено падение дерева
                t.sinkY -= 0.2; 
            }
        });
